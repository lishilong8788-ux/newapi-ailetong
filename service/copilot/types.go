...
type RunOptions struct {
	...
	// 它自己决定的写操作。
	ApprovedTool string
	// ApprovedArgs 是管理员点头时弹窗里显示的那份参数原值。
	//
	// 必须连参数一起对，不能只对工具名：批准的语义是重放整轮，而重放会重新问一次
	// 模型，模型是不确定的。只对名字的话，管理员看着 markup=0.3 点的头，重放时模型
	// 完全可以给出 markup=3.0 并被放行 —— handler 只兜边界，一个合法但不是他同意过
	// 的值会照写进库。
	//
	// 空表示不校验参数，只对名字。留这个退路是给非 HTTP 调用方（eval 测试）的，
	// 真实的确认流程一定带着它。
